...
function wave(__from, __to, __duration, __offset) {
	var a4 = (__to - __from) * 0.5;
	return __from + a4 + sin((((current_time * 0.001) + __duration * __offset) / __duration) * (pi * 2)) * a4;
}
...
/// @desc	Returns the length of a container, like a string or array. 0 is returned if an invalid container is given.
...
function odd(__x) {
	if (__x % 2 != 0) return true; else return false;
}
...
function distance(__x1, __y1, __x2, __y2) {
	return sqrt(sqr(__x2 - __x1) + sqr(__y2 - __y1));
}
...
function draw_overlay(__type, __color, __alpha, __x1 = 0, __y1 = 0, __x2 = window_get_width(), __y2 = window_get_height()) {
	...
	switch (__type) {
		// Part of the screen.
		case ov_part: draw_rectangle(__x1, __y1, __x2, __y2, 0); break;
		
		// Full screen.
		case ov_full: draw_rectangle(0, 0, window_get_width(), window_get_height(), 0); break;
